...
namespace Handshake
{
	namespace Video
	{
		...
		int32_t VGA::Init()
		{
			...
			// SETUP REGISTERS
			uint8_t IOAS = ((CPU::IO::INSB(EXT_MISC_OUT_READ_REG) >> 0) & 1);
			if(!IOAS)
			{
		...
		{
			uint8_t addrRegBak = CPU::IO::INSB(s_CtrlAddressRegister);
			CPU::IO::OUTB(s_CtrlAddressRegister, CTRL_CURSOR_START_REG);
			uint8_t cursorStartRegData = CPU::IO::INSB(s_CtrlDataRegister);
			ESTD::Bitwise::BitClear(cursorStartRegData, 5);
			CPU::IO::OUTB(s_CtrlDataRegister, cursorStartRegData);
			CPU::IO::OUTB(s_CtrlAddressRegister, addrRegBak);
		}
		...
		void VGA::DisableCursor()
		{
			...
			CPU::IO::OUTB(s_CtrlAddressRegister, CTRL_CURSOR_START_REG);
			uint8_t cursorStartRegData = CPU::IO::INSB(s_CtrlDataRegister);
			ESTD::Bitwise::BitSet(cursorStartRegData, 5);
			CPU::IO::OUTB(s_CtrlDataRegister, cursorStartRegData);
			CPU::IO::OUTB(s_CtrlAddressRegister, addrRegBak);
		}
		...
		void VGA::SetCursor(uint16_t offset)
		{
			// Low Bits
			uint8_t addrRegBak = CPU::IO::INSB(s_CtrlAddressRegister);
			CPU::IO::OUTB(s_CtrlAddressRegister, CTRL_CURSOR_LOCATION_LOW_REG);
			CPU::IO::OUTB(s_CtrlDataRegister, (offset & 0xFF));
			// High Bits
			CPU::IO::OUTB(s_CtrlAddressRegister, CTRL_CURSOR_LOCATION_HIGH_REG);
			CPU::IO::OUTB(s_CtrlDataRegister, ((offset >> 8) & 0xFF));
			CPU::IO::OUTB(s_CtrlAddressRegister, addrRegBak);
		}
		...
		uint16_t VGA::GetCursor()
		{
			...
			uint8_t addrRegBak = CPU::IO::INSB(s_CtrlAddressRegister);
			CPU::IO::OUTB(s_CtrlAddressRegister, CTRL_CURSOR_LOCATION_LOW_REG);
			offset = CPU::IO::INSB(s_CtrlDataRegister);
			// High Bits
			CPU::IO::OUTB(s_CtrlAddressRegister, CTRL_CURSOR_LOCATION_HIGH_REG);
			offset |= (CPU::IO::INSB(s_CtrlDataRegister) << 8);
			CPU::IO::OUTB(s_CtrlAddressRegister, addrRegBak);
			return offset;
		}

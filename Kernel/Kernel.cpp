...
namespace Kernel
{
	...
	bool InitPaging(const Handshake::BootInfo* bootInfo)
	{
		...
		// Identity Map first 4MB (DMA)
		uint32_t dma16PageCount = 0x400000 / PAGE_SIZE;
		if (!g_KernelPageManager->MapRange(0x0, 0x0, true, dma16PageCount))
		{
			KPrintf("InitPaging->PageManager Failed to Identity Map dma16!\n");
			return false;
		}
